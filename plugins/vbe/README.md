...
VBE stores its state in a file with the same name as the VBE method, with a
...
* compatible - the compatible string for VBE
...
New VBE methods can be added in the plugin file `fu-plugin-vbe.c` in the
`driver_list` array. See the documentation there for how to do this.
